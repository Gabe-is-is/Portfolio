<footer>
    <div class="footer-container">
        <div class="footer-container-grid">
            <div class="footer-container-grid-first_grid">
                <a href="#">Gabriely</a>
                <a href="#"><span>Gomes</span></a>
                <p>Desenvolvedora Frontend especializada em criar experiências web modernas e responsivas.</p>
            </div>

            <div class="footer-container-grid-second_grid">
                <h4>Links Rápidos</h4>
                <ul>
                    <li><a href="#">Sobre</a></li>
                    <li><a href="#">Projetos</a></li>
                    <li><a href="#">Habilidades</a></li>
                    <li><a href="#">Contrate-me</a></li>
                    <li><a href="#">Contato</a></li>
                </ul>
            </div>
            <div class="footer-container-grid-third_grid">
                <h4>Serviços</h4>
                <ul>
                    <li><a>Desenvolvimento Frontend</a></li>
                    <li><a>Otimização de Performance</a></li>
                    <li><a>Consultoria Web</a></li>
                    <li><a>Desenvolvimento Responsivo</a></li>
                </ul>
            </div>
            <div class="footer-container-grid-fourth_grid">
                <h4>Contato</h4>
                <ul>
                    <li><a href="#">{{ $contactInfo->location }}</a></li>
                    <li><a href="#">{{ $contactInfo->email }}</a></li>
                    <li><a href="#">{{ $contactInfo->phone }}</a></li>
                </ul>
                <div class="footer-container-grid-fourth_grid-svg">
                    <a href="#"><svg xmlns="http://www.w3.org/2000/svg" width="20px" height="20px"
                            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                            stroke-linecap="round" stroke-linejoin="round">
                            <path
                                d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4">
                            </path>
                            <path d="M9 18c-4.51 2-5-2-7-2"></path>
                        </svg>
                    </a>
                    <a href="#">
                        <svg xmlns="http://www.w3.org/2000/svg" width="20px" height="20px" viewBox="0 0 24 24"
                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                            stroke-linejoin="round">
                            <path
                                d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z">
                            </path>
                            <rect width="4" height="12" x="2" y="9"></rect>
                            <circle cx="4" cy="4" r="2"></circle>
                        </svg>
                    </a>
                    <a href="#"><svg xmlns="http://www.w3.org/2000/svg" width="20px" height="20px"
                            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                            stroke-linecap="round" stroke-linejoin="round">
                            <rect width="20" height="16" x="2" y="4" rx="2"></rect>
                            <path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"></path>
                        </svg>
                    </a>
                </div>
            </div>
        </div>
        <div class="footer-closing">
            <p>&copy; 2025 Gabriely Gomes. Todos os direitos reservados.</p>
        </div>
    </div>
</footer>

<!-- classes já refeitas -->